...
const TIMEOUT_FOR_VANILLA = 2000;

// WebRTCを利用する準備をする
function prepareNewConnection() {
  //const pc_config = {"iceServers":[ {"urls":"stun:stun.webrtc.ecl.ntt.com:3478"} ]}; // Cause long time for gathering ICE candidate in some network
  ...
  return new Promise(async (resolve, reject) =>  {
    ...
    peer.onnegotiationneeded = async () => {
      ...
      if (sendingOffer) {
        ...
        else if (iceType === ICETYPE_VANILLA) {
          setTimeout(()=>{
            if (! isResolved) {
              console.warn('---timeout for gathering ice candidate---');
              isResolved = true;
              resolve(peer.localDescription);
            }            
          }, TIMEOUT_FOR_VANILLA);
